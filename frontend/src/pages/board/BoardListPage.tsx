...
import { MessageSquare, Heart, Eye, Plus, Search, X } from 'lucide-react'
...
const PostCard = ({ post, onClick }: { post: PostSummary; onClick: () => void }) => (
  ...
  >
    <p className="text-[15px] font-semibold text-[#191F28] line-clamp-1 mb-1">
      {post.title}
    </p>
    {post.contentPreview && (
...
const BoardListPage = () => {
  ...
  const fetchPosts = useCallback(async (_page: number, reset: boolean, _keyword?: string) => {
    try {
      const res = await getPosts(_page, 20, _keyword || undefined)
